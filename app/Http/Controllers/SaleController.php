...
class SaleController extends Controller
{
    ...
    public function getProduct($id)
    {
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse) 
        {
            ...
            $product_name[] = $lims_product_data->size != "" ? htmlspecialchars($lims_product_data->name)."(".$lims_product_data->size.")" : htmlspecialchars($lims_product_data->name);
        ...
        foreach ($lims_product_data as $product) 
        {
            ...
            $product_name[] = $product->size != "" ? htmlspecialchars($product->name)."(".$product->size.")" : htmlspecialchars($product->name);
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            $lims_reward_point_setting_data = RewardPointSetting::latest()->first();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image','size')->ActiveFeatured()->whereNull('is_variant')->get();
            ...
            $lims_product_list_with_variant = Product::select('id', 'name', 'code', 'image','size')->ActiveFeatured()->whereNotNull('is_variant')->get();
            ...
            foreach ($lims_product_list_with_variant as $product) {
                ...
                if($product->size != ""){
                    $main_name = $product->name."(".$product->size.")";
                }else{
    ...
    public function getProductByFilter($category_id, $brand_id)
    {
        ...
        if(($category_id != 0) && ($brand_id != 0)){
            $lims_product_list = DB::table('products')
                                ...
                                ->where([
                                    ...
                                    ['products.is_active', true],
                                    ['brand_id', $brand_id]
                                ])->select('products.size','products.name', 'products.code', 'products.image')->get();
        }
        elseif(($category_id != 0) && ($brand_id == 0)){
            $lims_product_list = DB::table('products')
                                ...
                                ->where([
                                    ...
                                    ['categories.parent_id', $category_id],
                                    ['products.is_active', true]
                                ])->select('products.size','products.id', 'products.name', 'products.code', 'products.image', 'products.is_variant')->get();
        }
        elseif(($category_id == 0) && ($brand_id != 0)){
            $lims_product_list = Product::where([
                            ...
                            ->select('products.size','products.id', 'products.name', 'products.code', 'products.image', 'products.is_variant')
                            ->get();
        }
        else
        ...
        foreach ($lims_product_list as $product) {
            if($product->is_variant) {
                ...
                foreach ($lims_product_variant_data as $key => $variant) {
                    $data['name'][$index] = $product->size != "" ? $product->name."(".$product->size.")".' ['.$variant->name.']' : $product->name.' ['.$variant->name.']';
                ...
                $data['name'][$index] = $product->size != "" ? $product->name."(".$product->size.")" : $product->name;
